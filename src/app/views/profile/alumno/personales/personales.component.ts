...
export class PersonalesComponent implements OnInit {
  ...
  rellenarFormulario() {
    for (let llave in this.usuario_actual['datosPersonales'])
      if (llave in this.formulario.controls)
        //puede que hubiera campos en la BBDD que no se mostrasen por pantalla
        this.formulario.controls[llave].setValue(
          this.usuario_actual['datosPersonales'][llave]
        );
  }
  ...
  crearValidadores(): void {
    this.formulario.get('tipoDocumento').valueChanges.subscribe(result => {
      switch (result) {
        case 'Pasaporte':
          this.formulario
            .get('numeroDocumento')
            .setValidators([Validators.required, formatoPasaporte]);
          break;
        case 'NIF':
          this.formulario
            .get('numeroDocumento')
            .setValidators([Validators.required, formatoNIF]);
          break;
        case 'NIE':
          this.formulario
            .get('numeroDocumento')
            .setValidators([Validators.required, formatoNIE]);
          break;
      }
    });
  }
